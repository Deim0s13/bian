...
| `GAP-C-004` | Customer-to-BIAN interpretations are implicit or embedded in diagrams | Candidate, disputed, accepted, rejected and superseded relationship assertions retain rationale, evidence and decision history | Mapping disagreement becomes reviewable rather than hidden in notation | `REQ-003`, `REQ-007`; `DAR-007`, `DAR-008`, `DAR-012` | Mapping proposal and accountable review work packages |
| `GAP-C-005` | A decision's “current state” is reconstructed from whatever records are current when someone asks | Governed View Definitions and reconstructable decision-view packages retain or reproduce policy, time, inputs, result and limitations under `DEC-029` | A decision can be defended against the exact view used at the time | `REQ-006`, `REQ-007`; `DAR-004`, `DAR-026` | View-selection, materialisation and decision-package options |
